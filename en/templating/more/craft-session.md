...
(Note that checking for [`currentUser`]({entry:templating/global-variables}#currentUser) works just as well for this.)
...
Returns the user’s username, even if they are not logged in, if they have logged in [recently enough]({entry:docs/config-settings}#rememberUsernameDuration) for Craft to still remember it.
...
Returns the URL the user was trying to access before getting sent to the login page, because they hit a [`{% requireLogin %}`]({entry:templating/tags}#requireLogin) tag.
