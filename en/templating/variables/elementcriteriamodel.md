...
* Calling a relational field (e.g. an [Assets]({entry:docs/assets-fields}) field) from an element will give you an ElementCriteriaModel with its [`relatedTo`]({entry:docs/relations}#the-relatedTo-param) parameter pre-populated.

    ```twig
    {% set assets = entry.myAssetsField %}
    ```

* Calling a category’s [`getChildren()`]({entry:templating/categorymodel}#getChildren) function will give you an ElementCriteriaModel with its [`descendantOf`]({entry:templating/craft.categories}#descendantOf) and [`descendantDist`]({entry:templating/craft.categories}#descendantDist) parameters pre-populated.
